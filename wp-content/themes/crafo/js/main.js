...
(function ($) {
    ...
    $(document).on('click', '.submit', function () {
        ...
        var form_submit_btn = _this;
        formObj.find('.required').removeClass('is-invalid');
        formObj.find('.required:input').each(function () {
            var __this = $(this),
                    fieldVal = __this.val();
            if (fieldVal == '') {
                error = true;
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (redirectVal != '' && redirectVal != undefined) {
                        ...
                        resultsObj.addClass('alert-success').html('Comment submitted successfully!');
                        resultsObj.removeClass('d-none').fadeIn('slow').delay(4000).fadeOut('slow');
                        window.location.href = redirectVal;
                    } else {
                        if (typeof (result) !== 'undefined' && result !== null) {
                            if (typeof result === 'object' && result.hasOwnProperty('data')) {
                                result.alert = result.success == true ? 'alert-success' : 'alert-danger';
                ...
                error: function(jqXHR, textStatus, errorThrown) {
                    ...
                    if (statusCode === 409) {
                        resultsObj.removeClass('alert-danger').removeClass('alert-success').hide();
                        resultsObj.addClass('alert-warning').html('Duplicate comment detected; it looks as though you’ve already said that!');
                        resultsObj.removeClass('d-none').fadeIn('slow').delay(4000).fadeOut('slow');
                    }
                }
            });
